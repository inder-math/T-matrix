...
C   and must be used along with the accompanying file tmq.par.f.          
...
C   The computational method is based on the T-matrix approach and     
C   is described in the following papers:                              
C                                                                      
...
C   Mishchenko.  Please e-mail your request to crmim@giss.nasa.gov.      
...
C   particles with sharp rectangular edges.  Ref. 5 describes most     
...
C   is met. It is completely independent of the first                     
C   part and contains no T-matrix-specific subroutines, so
C   that it can be compiled separately and only once.               
...
C   As described in Ref. 5 above, the use of the LU factorization      
...
C   code can still be used, but needs the following changes:           
C                                                                      
...
C                                                                      
C   As described in Ref. 5 above, these changes will affect the        
...
C             by equations (1)-(3) of Ref. 7] versus scattering angle
...
C   ANY DAMAGES THAT MAY RESULT FROM THE USE OF THE PROGRAM. 
     ...
     *        DDR(NPNG2),DRR(NPNG2),DRI(NPNG2),ANN(NPN1,NPN1)
     ...
     &     IT21(NPN6,NPN4,NPN4),IT22(NPN6,NPN4,NPN4)
      ...
      CALL GET_COMMAND_ARGUMENT(1, EPS_STRING)
      READ(EPS_STRING,*) EPS


C  OPEN FILES *:******************************************************

      OPEN(6,FILE='/my-projects/stetson/singh/jv_pro_adv/r_0.8/output/
     &300nm/ar_'//TRIM(EPS_STRING)//'.dat')
      OPEN (10,FILE='/my-projects/stetson/singh/jv_pro_adv/r_0.8/expan
     &_co/300nm/ar_'//TRIM(EPS_STRING)//'.dat')
      open (20,file='/my-projects/stetson/singh/jv_pro_adv/r_0.8/scat
     &_mat/300nm/ar_'//TRIM(EPS_STRING)//'.dat')
...
c         R2=2.0D0*AXI
 ...
 8002    FORMAT('R1=',F10.6,'   R2=',F10.6)
         IF (DABS(RAT-1D0).LE.1D-6) Write(6,8003) REFF,VEFF
c         Write(6,8003) REFF,VEFF
         IF (DABS(RAT-1D0).GT.1D-6) Write (6,8004) REFF,VEFF
c         write(6,8004) REFF,VEFF
...
C           PRINT 6070, I,NMAX,NMAX1,NGAUSS
 6070       FORMAT(4I6)
  ...
  510    CONTINUE
         ...
         ASYMM=ALPH1(2)/3D0
         WRITE(6,9100)CEXTIN,CSCAT,WALB,ASYMM
...
C        180*(I-1)/(N-1) (DEGREES), WHERE I NUMBERS THE ANGLES
 ...
 1001 FORMAT(' ',2X,'S',6X,'ALPHA1',6X,'ALPHA2',6X,'ALPHA3',
     &       6X,'ALPHA4',7X,'BETA1',7X,'BETA2')
      DO 10 L1=1,L1MAX
         L=L1-1
         write(10,1002)L,A1(L1),A2(L1),A3(L1),A4(L1),B1(L1),B2(L1)
...
c        F12=-F12/F11
c        F34=F34/F11
         write(20,1004)TB,F11,F22,F33,F44,F12,F34
